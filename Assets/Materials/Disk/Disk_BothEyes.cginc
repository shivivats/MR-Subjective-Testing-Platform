...
void Geometry(point Varyings input[1], inout TriangleStream<VertOut> outStream)
{
	...
	UNITY_LOOP for (uint i = 1; i < slices; i++)
	{
		...
		// Right side vertex
		vo.position.xy = origin.xy + extent * float2(sn, cs);
		vo.uv = float2(sn, cs);
		vo.viewposition = mul(_InverseProjMatrix, vo.position);
		vo.viewposition /= vo.viewposition.w;
		outStream.Append(vo);
		...
		// Left side vertex
		vo.position.x = origin.x - extent.x * sn;
		vo.uv.x = -sn;
		vo.viewposition = mul(_InverseProjMatrix, vo.position);
		vo.viewposition /= vo.viewposition.w;
		outStream.Append(vo);
...
FragOut Fragment(VertOut input)
{
	...
	if (_ShaderInterpolation > 0.5) {
		...
		input.viewposition.z += (1 - uvlen) * input.size * _PointScalingFactor * _ModelScalingFactor * 2.0f;
	}
